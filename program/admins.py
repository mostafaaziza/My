...
async def stop(client, m: Message):
    ...
    if chat_id in QUEUE:
        try:
            ...
            clear_queue(chat_id)
            await m.reply_text("⋆ تم ايقاف الاغنيه •")
        except Exception as e:
            traceback.print_exc()
            await m.reply_text(f"🚫 **error:**\n\n`{e}`")
    else:
        await m.reply_text("⋆ مفيش حاجه شغاله اصلا •")
...
@Client.on_message(
    command(["كمل", f"unmute@{BOT_USERNAME}", "vunmute"]) & other_filters
)
@authorized_users_only
@check_blacklist()
...
async def cbpause(_, query: CallbackQuery):
    ...
    if not a.can_manage_voice_chats:
        return await query.answer("⋆ هذا الامر ليس لك •!", show_alert=True)
...
async def cbstop(_, query: CallbackQuery):
    ...
    if chat_id in QUEUE:
        try:
            ...
            await remove_active_chat(chat_id)
            clear_queue(chat_id)
            await query.edit_message_text("⋆ تم انهاء الكل •.", reply_markup=close_mark)
        except Exception as e:
            traceback.print_exc()
            await query.edit_message_text(f"🚫 **error:**\n\n`{e}`", reply_markup=close_mark)
    else:
...
async def cbskip(_, query: CallbackQuery):
    ...
    if queue == 0:
        ...
        await query.answer("⋆ جاري تشغيل الاغنيه التاليه •")
